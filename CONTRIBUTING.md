...
Thanks for being interested in BasslineMnk! This document is meant to help developers get up to speed on BasslineMnk so that they can start development with as little frustration as possible.

## Setting up a development environment

The steps below show you how to quickly set up a development environment for BasslineMnk.
...
To install BasslineMnk's dev packages, run the following command:
...
To run BasslineMnk's build scripts, run:
...
If you're planning to contribute code to BasslineMnk, it's a good idea to enable the standard Git hooks so that build scripts run before you commit. That way, you can see if basic tests pass in a few seconds rather than waiting a few minutes to watch them run in CircleCI.
...
The BasslineMnk server backend uses several privileged scripts (created in [ansible-role-basslinemnk](https://github.com/bassline-mnk/ansible-role-basslinemnk)). Those scripts exist on a provisioned BasslineMnk device, but they don't exist on a dev machine.
...
To run BasslineMnk on a non-Pi machine, run:
...
In case you need SSH keys for accessing the Git repositories (e.g., for testing BasslineMnk's Pro version), please enable SSH agent forwarding in your local `~/.ssh/config`.
...
For a high-level view of BasslineMnk's architecture, see the [ARCHITECTURE](ARCHITECTURE.md) file.

## Options

BasslineMnk accepts various options through environment variables:
...
BasslineMnk follows Google code style conventions:
...
BasslineMnk uses automated linters and formatters as much as possible to automate style conventions.

## Proposing changes

* If you're making a small change, submit a PR to show your proposal.
* If you're making a large change (over 100 LOC or three hours of dev time), [file an issue](https://github.com/bassline-mnk/basslinemnk/issues/new/choose) first to talk through the proposed change. This prevents you from wasting time on a change that has a low chance of being accepted.

## How to get your PR merged quickly

* Read my guide, ["How to Make Your Code Reviewer Fall in Love with You,"](https://adrian-griffin.io/code-review-love/) to understand how to contribute effectively to an open source project.
